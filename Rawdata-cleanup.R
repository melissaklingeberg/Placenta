...
###    2. DATA PREPERATION ----
#### 2.1 Formatting ----
Annotation_Rawdata[ Annotation_Rawdata == "NA" ] <- NA                #change character "NA" to NA

cleaned_data <- Annotation_Rawdata  %>%
  separate(identifiers,
           c("Observation", "Celltype", "ID", "Time"),
           sep = "_",
           remove = TRUE) %>%                                         #separate first column into four columns
  gather(Antibody, value, 5:ncol(.)) %>%                              #reshape "AB" columns to rows
  spread(Observation, value) %>%                                      #spread "Observation" into 3 separate columns
  mutate(Qt = as.numeric(Qt), 
         Int = ifelse(as.numeric(Int)<=1, "Undistinct", "Distinct"))  #change chr to num in columns + change format of Intensity
...
tissue_QC <- cleaned_data %>%
  group_by(ID, Antibody) %>%                                #summarize patients in a group
  summarise(NA_count = sum(is.na(Int))) %>% ungroup() %>%   #count cores, where no cells (NA=4) could be found
  filter(NA_count == total_cell) %>%
  count(ID) %>%  
  mutate(Per_Missing = n/total_ab*100) %>%                  #create new column with percentage of empty cores, filter for those with more than 25% empty
...
write.table(tissue_QC, file = "Outlier_tissue.txt", sep = "\t")      #export table

filtered_tissue <- cleaned_data %>%                                  #delete outlier(s) from table
...
### 6. Manual annotation - trophoblast specificity ----
tspec_raw <- filter_tiss_ab %>%
inner_join(tropho_man, by = "Antibody")

### 7. AB - Celltype specificity ----
#### 7.1 Filtering ----
AB_Venn <- tspec_raw %>% 
  filter(Int == "Distinct") %>%
...
#### 8. MARKER PROTEINS FOR SPECIFIC TIMEPOINT OF PLACENTAL DEVELOPMENT ----
